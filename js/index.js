    ...
    reminder.controller('mainCtrl',['$scope',function($scope){
        ...
        $scope.change=function(t){ //改变主题颜色
            $scope.current.them = t.theme;
        }

        $scope.count=function(){  //已完成数量
            var index=0;
        ...
        $scope.cancel=function(){  //卡片出现取消
            $(".card").toggleClass("active")
        }
        $scope.deletecard=function(){
            $(".card").removeClass("active")
        }
        $scope.addtodo=function(e) {// 左边增加新建项目
                var max_id = -100;
                angular.forEach($scope.current.todo, function (v, i) {
                    if (v.id > max_id) {
                        max_id = v.id;
        ...
        $scope.shanchu=function(dd){  //卡片里的删除
            var newarr=[];
        ...
        $scope.delete1=function(dd){ //新建项目的删除
            var newtodo=[];
